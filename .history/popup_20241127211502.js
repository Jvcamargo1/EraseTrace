      ...
      chrome.storage.sync.get(['sitesToDelete'], function(result) {
        ...
        if (result.sitesToDelete && result.sitesToDelete.length > 0) {
          result.sitesToDelete.forEach((site, index) => {
            ...
            siteItem.innerHTML = `
              <input type="checkbox" id="site${index}"// ... (código existente)
              
              document.getElementById('clearHistory').addEventListener('click', function() {
                const selectedSites = Array.from(document.querySelectorAll('#sitesList input[type="checkbox"]:checked'))
                  .map(checkbox => checkbox.value);
              
                chrome.runtime.sendMessage({action: "clearHistory", sites: selectedSites});
              });
              
              // Adicione este listener para receber a mensagem do background
              ...
              // ... (resto do código existente) checked>
